'''
Module for gathering info from Wolf Heating System via ISM8 adpater
'''
...
datapoints = {
    ...
    9: ('HG1', 'Status Brenner / Flamme', 'DPT_Switch', False, ''),
    10: ('HG1', 'Status Heizkreispumpe', 'DPT_Switch', False, ''),
    11: ('HG1', 'Status Speicherladepumpe', 'DPT_Switch', False, ''),
    12: ('HG1', 'Status 3-Wege-Umschaltventil', 'DPT_OpenClose', False, ''),
    ...
    22: ('HG2', 'Status Brenner / Flamme', 'DPT_Switch', False, ''),
    23: ('HG2', 'Status Heizkreispumpe', 'DPT_Switch', False, ''),
    24: ('HG2', 'Status Speicherladepumpe', 'DPT_Switch', False, ''),
    25: ('HG2', 'Status 3-Wege-Umschaltventil', 'DPT_OpenClose', False, ''),
    ...
    35: ('HG3', 'Status Brenner / Flamme', 'DPT_Switch', False, ''),
    36: ('HG3', 'Status Heizkreispumpe', 'DPT_Switch', False, ''),
    37: ('HG3', 'Status Speicherladepumpe', 'DPT_Switch', False, ''),
    38: ('HG3', 'Status 3-Wege-Umschaltventil', 'DPT_OpenClose', False, ''),
    ...
    48: ('HG4', 'Status Brenner / Flamme', 'DPT_Switch', False, ''),
    49: ('HG4', 'Status Heizkreispumpe', 'DPT_Switch', False, ''),
    50: ('HG4', 'Status Speicherladepumpe', 'DPT_Switch', False, ''),
    51: ('HG4', 'Status 3-Wege-Umschaltventil', 'DPT_OpenClose', False, ''),
    ...
    109: ('KM', 'Vorlauftemperatur Mischerkreis', 'DPT_Value_Temp', False, 'C'),
    ...
    116: ('MM1', 'Vorlauftemperatur Mischerkreis', 'DPT_Value_Temp', False, 'C'),
    ...
    123: ('MM2', 'Vorlauftemperatur Mischerkreis', 'DPT_Value_Temp', False, 'C'),
    ...
    130: ('MM3', 'Vorlauftemperatur Mischerkreis', 'DPT_Value_Temp', False, 'C'),
    ...
    139: ('SM', 'Eingang E2 (Durchfluss)', 'DPT_Value_Volme_Flow', False, 'l/h'),
    ...
    153: ('CWL', 'Intensivlueftung AN/AUS', 'DPT_Switch', True, ''),
    ...
    158: ('CWL', 'Zeitweiser Feuchteschutz AN/AUS', 'DPT_Switch', True, ''),
    159: ('CWL', 'Zeitweiser Feuchteschutz Startdatum', 'DPT_Date', True, ''),
    160: ('CWL', 'Zeitweiser Feuchteschutz Enddatum', 'DPT_Date', True, ''),
    161: ('CWL', 'Zeitweiser Feuchteschutz Startzeit', 'DPT_TimeOfDay', True, ''),
    162: ('CWL', 'Zeitweiser Feuchteschutz Endzeit', 'DPT_TimeOfDay', True, ''),
    ...
    166: ('CWL', 'Luftdurchsatz Zuluft', 'DPT_FlowRate_m3/h', False, 'ccm/h'),
    167: ('CWL', 'Luftdurchsatz Abluft', 'DPT_FlowRate_m3/h', False, 'ccm/h'),
    168: ('CWL', 'Bypass Initialisierung', 'DPT_Bool', False, ''),
    169: ('CWL', 'Bypass oeffnet/offen', 'DPT_Bool', False, ''),
    170: ('CWL', 'Bypass schliesst/geschlossen', 'DPT_Bool', False, ''),
    171: ('CWL', 'Bypass Fehler', 'DPT_Bool', False, ''),
    172: ('CWL', 'Frost Status: Initialisierung/Warte', 'DPT_Bool', False, ''),
    173: ('CWL', 'Frost Status: Kein Frost', 'DPT_Bool', False, ''),
    174: ('CWL', 'Frost Status: Vorwaermer', 'DPT_Bool', False, ''),
    175: ('CWL', 'Frost Status: Fehler/Unausgeglichen', 'DPT_Bool', False, ''),
    ...
    186: ('BWL', 'Status Zubringer-/Heizkreispumpe', 'DPT_Switch', False, ''),
    187: ('BWL', '3-Wege-Umschaltventil HZ/WW', 'DPT_OpenClose', False, ''),
    188: ('BWL', '3-Wege-Umschaltventil HZ/K', 'DPT_OpenClose', False, ''),
...
def decode_datapoint(self, dp_id, length, raw_bytes):
    ...
    if dp_id not in Ism8.datapoints:
        _LOGGER.error("unknown datapoint: %s, data:%s",
                      dp_id, result)
        return
    ...
    if (length == 1) and (dp_type in ("DPT_Switch",
        ...
        Ism8.dp_values.update({dp_id: bool(result & 1)})

    elif (length == 1) and (dp_type == "DPT_HVACMode"):
        # translate values to clear status-text
        if result == 0:
            Ism8.dp_values.update({dp_id: 'Auto'})
        elif result == 1:
            Ism8.dp_values.update({dp_id: 'Comfort'})
        elif result == 2:
            Ism8.dp_values.update({dp_id: 'Standby'})
        elif result == 3:
            Ism8.dp_values.update({dp_id: 'Economy'})
        elif result == 4:
            Ism8.dp_values.update({dp_id: 'Building Protection'})

    elif (length == 1) and (dp_type == "DPT_Scaling"):
        # take byte value and multiply by 100/255
        Ism8.dp_values.update({dp_id: 100 / 255 * result})

    elif (length == 1) and (dp_type == "DPT_DHWMode"):
        if result == 0:
            Ism8.dp_values.update({dp_id: 'Auto'})
        elif result == 1:
            Ism8.dp_values.update({dp_id: 'LegioProtect'})
        elif result == 2:
            Ism8.dp_values.update({dp_id: 'Normal'})
        elif result == 3:
            Ism8.dp_values.update({dp_id: 'Reduced'})
        elif result == 4:
            Ism8.dp_values.update({dp_id: 'Off'})
        ...
        if result == 0:
            ...
            Ism8.dp_values.update({dp_id: 'Heat'})
        elif result == 2:
            Ism8.dp_values.update({dp_id: 'Morning Warmup'})
        elif result == 3:
            Ism8.dp_values.update({dp_id: 'Cool'})
        elif result == 4:
            Ism8.dp_values.update({dp_id: 'Night Purge'})
        elif result == 5:
            Ism8.dp_values.update({dp_id: 'Precool'})
        elif result == 6:
            Ism8.dp_values.update({dp_id: 'Off'})
        elif result == 7:
            Ism8.dp_values.update({dp_id: 'Test'})
        elif result == 8:
            Ism8.dp_values.update({dp_id: 'Emergency Heat'})
        elif result == 9:
            Ism8.dp_values.update({dp_id: 'Fan Only'})
        elif result == 10:
            Ism8.dp_values.update({dp_id: 'Free Cool'})
        elif result == 11:
            Ism8.dp_values.update({dp_id: 'Ice'})
        elif result == 12:
            Ism8.dp_values.update({dp_id: 'Maximum Heating Mode'})
        elif result == 13:
            Ism8.dp_values.update({dp_id: 'Economic Heat/Cool Mode'})
        elif result == 14:
            Ism8.dp_values.update({dp_id: 'Dehumidification'})
        elif result == 15:
            Ism8.dp_values.update({dp_id: 'Calibration Mode'})
        elif result == 16:
            Ism8.dp_values.update({dp_id: 'Emergency Cool Mode'})
        elif result == 17:
            Ism8.dp_values.update({dp_id: 'Emergency Steam Mode'})
        elif result == 20:
            Ism8.dp_values.update({dp_id: 'NoDem'})
        ...
        _LOGGER.debug('binary format {0:b} -> s:{1:b} ,  m:{2:b} , e:{3:b}'
                      .format(result, _sign, _mantisse, _exponent))
        if _sign == 1:
            _mantisse = -(~(_mantisse - 1) & 0x07ff)
        Ism8.dp_values.update({dp_id: (0.01 * (2 ** _exponent) * _mantisse)})
    else:
        _LOGGER.error('datatype not implemented: %s ', dp_type)
        return
    _LOGGER.debug('decoded DP %s : %s = %s\n',
                  dp_id, Ism8.datapoints[dp_id], Ism8.dp_values[dp_id])
...
class Ism8(asyncio.Protocol):
    ...
    def __init__(self):
        self._dp_values = {}
        # all the datapoint-values (IDs matching the list above) are stored here
    ...
    # is called as soon as an ISM8 connects to server
        ...
        _LOGGER.info("Connection from ISM8: %s", _peername)
    ...
    # is called whenever data is ready
        ...
        while _header_ptr < len(data):
            ...
            # buffer is larger => than msg: process 1 message, then continue loop
            if len(data) < _header_ptr + msg_length:
                _LOGGER.debug("Buffer shorter than expected / broken Message.")
                _LOGGER.debug("Discarding: %s ", data[_header_ptr:])
                # setting Ptr to end of data will end loop
                _header_ptr = len(data)
            else:
                # send ACK to ISM8 according to API: ISM_Header,
                ...
                _LOGGER.debug('Sending ACK: %s ', ack_msg)
                self._transport.write(ack_msg)
                process_msg(data[_header_ptr + 10:_header_ptr + msg_length])
                # process message without header (first 10 bytes)
                _header_ptr = _header_ptr + msg_length
                # prepare to get next message; advance Ptr to next Msg

    def connection_lost(self, exc):
        _LOGGER.debug('ISM8 closed the connection.Stopping')
        self._connected = False
    ...
    def read_sensor_value(self, dp_id):
        if (dp_id in dp_values.keys()):
            return dp_values(dp_id)
        else:
            return None      
...
if __name__ == "__main__":
    ...
    myProtocol=Ism8()
    for keys, values in myProtocol.list_all_sensors().items():
        print ("Datapoint: %s \n %s\n" %(keys, values))
